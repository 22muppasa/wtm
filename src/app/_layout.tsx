...
import {LogoMark,T} from '@/components/ui';
import Animated,{Easing,useAnimatedStyle,useReducedMotion,useSharedValue,withRepeat,withTiming} from 'react-native-reanimated';

export {ErrorBoundary} from 'expo-router';
function BootScreen(){const{colors}=useTheme();const reduced=useReducedMotion();const turn=useSharedValue(0);useEffect(()=>{if(!reduced)turn.value=withRepeat(withTiming(360,{duration:1150,easing:Easing.inOut(Easing.cubic)}),-1,false);},[reduced,turn]);const logoStyle=useAnimatedStyle(()=>({transform:[{rotate:`${turn.value}deg`},{scale:1+.04*Math.sin(turn.value*Math.PI/180)}]}));return <View style={{flex:1,alignItems:'center',justifyContent:'center',gap:26,backgroundColor:colors.background}}><Animated.View style={logoStyle}><LogoMark size={116}/></Animated.View><View style={{alignItems:'center',gap:7}}><T variant="title">WTM</T><T color={colors.textSecondary}>Getting your world ready…</T></View></View>;}
